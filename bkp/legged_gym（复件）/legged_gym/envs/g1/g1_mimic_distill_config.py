...
class G1MimicPrivCfg(HumanoidMimicCfg):
    ...
    class motion(HumanoidMimicCfg.motion):
        ...
        motion_file = f"/home/jaxon/TWIST-master/motion_data_configs/twist_dataset.yaml"
...
# ============================================================================
# 专用于行走和跑步的学生模型配置 (Walk & Run Student Model)
# ============================================================================
class G1MimicStuWalkRunCfg(G1MimicStuRLCfg):
    """
    学生模型专用配置：仅训练行走和跑步动作
    Student model specialized for walking and running motions only
    """
    class env(G1MimicStuRLCfg.env):
        num_envs = 256  # 更小的环境数，因为数据集更简单
    
    class motion(G1MimicStuRLCfg.motion):
        motion_file = f"/home/jaxon/TWIST-master/motion_data_configs/walk_run_dataset.yaml"
        motion_curriculum = True
        motion_curriculum_gamma = 0.01
